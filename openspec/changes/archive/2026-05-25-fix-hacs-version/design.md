...
Current state:
- `manifest.json` has `"version": "0.8.0"` but no corresponding git tag exists
...
HACS reads the version from git tags. The tag format should be plain semver without `v` prefix (e.g. `0.8.0`) to match `manifest.json` directly.

**Alternative considered:** Using `v`-prefixed tags (e.g. `v0.8.0`) — this also works but requires HACS to strip the prefix. Plain semver is simpler and avoids mismatch.
