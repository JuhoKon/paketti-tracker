...
### Requirement: Notification dispatch via HA REST API

The system SHALL send notifications by calling the HA REST API at http://supervisor/core/api/services/notify/{device} using SUPERVISOR_TOKEN for authentication, instead of using in-process hass.services.async_call.

#### Scenario: Notification sent via HTTP
- **WHEN** a notification needs to be dispatched
- **THEN** the system SHALL make an HTTP POST to http://supervisor/core/api/services/notify/{device} with Bearer token authentication

#### Scenario: No dependency on hass object
- **WHEN** the notification service is initialized
- **THEN** it SHALL NOT require a reference to the hass object or use hass.services.async_call
...
- **WHEN** the system detects a package status transition matching a configured trigger (e.g. `pending` -> `in_transit`)
- **THEN** a notification SHALL be sent to all configured devices with the package name, new status, and last event description
