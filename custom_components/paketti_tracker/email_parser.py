...
# Posti: JJFI + 9 digits or uppercase letters/digits of length 13-20 starting with JJFI
_POSTI_PATTERNS: list[re.Pattern[str]] = [
    re.compile(r"\b(JJFI\d{9,16})\b", re.IGNORECASE),
    # Generic Posti tracking code (2 letters + 9 digits + 2 letters)
    re.compile(r"\b([A-Z]{2}\d{9}FI)\b", re.IGNORECASE),
]

# Postnord: Typically 14-20 digit numbers or starts with specific prefixes
_POSTNORD_PATTERNS: list[re.Pattern[str]] = [
    re.compile(r"\b(\d{14,20})\b"),
]

# Matkahuolto: typically numeric, 10-15 digits
...
def _extract_tracking_ids(text: str, vendor: str | None) -> list[tuple[str, str]]:
    ...
    # If vendor is known, only search with that vendor's patterns.
        ...
        for pattern in _MATKAHUOLTO_PATTERNS:
            for match in pattern.finditer(text):
                ...
                if tid not in seen:
                    seen.add(tid)
                    results.append((tid, VENDOR_MATKAHUOLTO))
        # Postnord digit-only patterns are very broad — only use if sender hints.
